#!/usr/bin/env python3
"""Toy neural-network experiment for predicting SPY's next daily direction."""
...
def download_spy(start: str) -> pd.DataFrame:
    """Download adjusted daily SPY history from Yahoo Finance."""
    data = yf.download("SPY", start=start, auto_adjust=True, progress=False)
    if data.empty:
        raise RuntimeError("No SPY data downloaded. Check your network connection.")
...
def run_backtest(
    ...
    """Compare buy-and-hold with simple daily option-buying strategies."""
...
def plot_equity_curve(backtest: pd.DataFrame, output_path: str) -> None:
    ...
    plt.plot(backtest.index, backtest["buy_hold_equity"], label="Buy and hold SPY")
    ...
    plt.title("SPY Toy Strategy Equity Curve")
    plt.xlabel("Date")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a toy PyTorch model on SPY daily data.")
    ...
    parser.add_argument("--plot", default="equity_curve.png", help="Output path for equity curve plot.")
...
def main() -> None:
    ...
    print(f"Latest predicted probability of SPY closing higher tomorrow: {latest_probability:.3f}")
